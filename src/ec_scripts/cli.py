...
from .output.pipeline import parse_paper_only, tidy_up_paper_folder
...
def main() -> None:
    ...
    if root_path.is_dir():
        ...
        paper= parse_paper_only(root_path).unwrap()
        paper.decode_json(out_path, out_path.with_name(f"{out_path.name}_warnings.json"))
